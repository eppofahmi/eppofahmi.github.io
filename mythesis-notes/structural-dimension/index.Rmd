...
tagar %>%
  arrange(desc(n)) %>%
  top_n(20) %>%
  ggplot(aes(x = reorder(hashtag, n), y = n, fill = "fase")) +
...
user_terlibat %>%
  arrange(desc(n)) %>%
  top_n(10) %>%
  ggplot(aes(x = reorder(user_all, n), y = n, fill = "fase")) +
  geom_col(show.legend = FALSE) +
  coord_flip() +
  facet_wrap(~fase) +
...
1. jumlah twit per hari
2. jumlah user per hari
3. jumlah fav, ret, dan fav
